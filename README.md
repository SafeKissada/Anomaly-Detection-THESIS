...
**ตั้งแต่การแยก path รอบล่าสุด**: `Config.SAVE_PATH` เก็บ **ตัวเลข/log/checkpoint ทั้งหมด** ส่วน `Config.OUTPUT_PATH` เก็บ **ไฟล์ภาพ (.png) ทั้งหมดเท่านั้น** — ไม่มีไฟล์ตัวเลขปนอยู่กับภาพอีกต่อไป ทั้งสองโฟลเดอร์มี `README.md` ที่ `src/output_docs.py` สร้างให้อัตโนมัติหลังรันเสร็จ อธิบายว่าแต่ละไฟล์เก็บอะไรไว้บ้างและเอาไปทำอะไรต่อได้บ้างแบบละเอียด (dynamic — list เฉพาะไฟล์ที่มีอยู่จริง ณ ตอนรัน)

**`Config.SAVE_PATH`** (ตัวเลข/log — เขียนโดย `scripts/train.py`):
...
- `final_results.json` — สรุปผล run แบบเต็ม (snapshot ของทุก field ใน Config ผ่าน `dataclasses.asdict()` บวก metrics บวก `naive_baselines`) สำหรับเทียบ experiment
- Checkpoint ของ autoencoder ที่เทรนแล้ว (`best_autoencoder.pth`, `autoencoder_final.pth` — คนละไฟล์กัน ดู `README.md` ใน `SAVE_PATH` สำหรับความต่าง)
- `roc_curve_data_val.csv`, `roc_curve_data_test.csv` — จุดดิบทุกจุดบนเส้น ROC (`fpr`, `tpr`, `threshold`) เขียนโดย `scripts/visualize.py` — ใช้เปิดใน Excel หรือเป็น input ของ multi-seed ROC aggregation ในอนาคต
- `README.md` — เอกสารอธิบายไฟล์ทั้งหมดข้างบน (auto-generated)
- ไฟล์ optional ที่มีก็ต่อเมื่อเคยรัน script เสริม: `gallery_index.csv`, `cost_aware_sweep.csv` (จาก `scripts/run_cost_aware.py`), `features_{split}.npz` (จาก `scripts/export_features.py`)

**`Config.OUTPUT_PATH`** (ภาพล้วน — เขียนโดย `scripts/visualize.py`):

- `roc_curves.png`, `pr_curves.png`, `confusion_matrices.png`, `score_distributions.png`, `training_history.png`, `eda_class_distribution.png`
- `heatmaps_{split}*.png` — ตัวอย่าง heatmap ต่อ split
- `gallery_*.png` — gallery หลายแบบ (ภาพต้นฉบับ/ภาพ+overlay/preprocessed/กรองตามผลทาย)
- `README.md` — เอกสารอธิบายไฟล์ภาพทั้งหมดข้างบน (auto-generated)
...
**As of the latest path-separation pass**: `Config.SAVE_PATH` holds **all numeric/log/checkpoint files**, while `Config.OUTPUT_PATH` holds **image (.png) files only** — no numeric files are mixed in with images anymore. Both folders get a `README.md`, auto-generated by `src/output_docs.py` after each run, documenting in detail what every file holds and what it can be used for (dynamic — only lists files that actually exist at run time).

**`Config.SAVE_PATH`** (numeric/log — written by `scripts/train.py`):
...
- `final_results.json` — full run summary (a complete `Config` field snapshot via `dataclasses.asdict()`, plus metrics, plus `naive_baselines`), for comparing experiments without cross-referencing `config.py`
- Trained autoencoder checkpoints (`best_autoencoder.pth`, `autoencoder_final.pth` — two different files, see the `README.md` in `SAVE_PATH` for the distinction)
- `roc_curve_data_val.csv`, `roc_curve_data_test.csv` — raw ROC curve points (`fpr`, `tpr`, `threshold`), written by `scripts/visualize.py` — usable in Excel or as input for future multi-seed ROC aggregation
- `README.md` — documentation for everything above (auto-generated)
- Optional files, present only if the relevant script was run: `gallery_index.csv`, `cost_aware_sweep.csv` (from `scripts/run_cost_aware.py`), `features_{split}.npz` (from `scripts/export_features.py`)

**`Config.OUTPUT_PATH`** (images only — written by `scripts/visualize.py`):

- `roc_curves.png`, `pr_curves.png`, `confusion_matrices.png`, `score_distributions.png`, `training_history.png`, `eda_class_distribution.png`
- `heatmaps_{split}*.png` — sample heatmaps per split
- `gallery_*.png` — several gallery variants (original / with overlay / preprocessed / filtered by prediction outcome)
- `README.md` — documentation for everything above (auto-generated)
